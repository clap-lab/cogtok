...
results.rename(columns = {'Reading Time':'Response Time'}, inplace = True)
...
axes[0].xaxis.label.set_text("Words")
...
results['Metric'] = results['Metric'].str.replace('Reading Time','Response Time')
ax = seaborn.catplot( data=results, x= "Model", y="Correlation",kind ="bar", hue="Language", col="Metric",col_order = ["Accuracy", "Response Time"],   palette=palette3 , legend=True, legend_out=False)
seaborn.move_legend(ax, "upper right", bbox_to_anchor=(0.9, .9))
