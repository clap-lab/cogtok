...
from train_tokenization import tokenize, Tokenizer
...
def evaluate_derivational_coverage(language, vocabfile):
    derivational = read_derivational(language)
    derivational_morphemes = set(derivational["morpheme"])
...
def get_derivation(sequence, derivational_segmentation):
    #print(derivational_segmentation.keys())
...
modelfile =  "../results/en/models/BPE_50000/trained.json"
...
#
#
#             monolingual_path = outdir + language + "/models/" + alg + "_" + str(vocab_size)
#             multilingual_path = outdir +  "crosslingual/models/" + alg + "_" + str(vocab_size)
#
#             print("Monolingual")
#             with open( monolingual_path+ "/vocabulary.txt", "r") as vocabfile:
#                 covered, coverage = evaluate_derivational_coverage(language_codes[language], vocabfile)
...
#         break
        ...
        # TODO 2: for morphological segmentation derivational and inflectional entries need to be combined -> check how to do that

# TODO 3: compare "gold standard" to tokenizer output

# TODO 4: lowercase everything before tokenization?